...
const STATE_DIR = join(process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude'), 'LIFEOS/MEMORY/STATE/isa-render-debounce');
const ISA_RENDER = join(process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude'), 'LIFEOS/TOOLS/ISARender.ts');
...
if (rendered.length || skipped.length) {
  try {
    ...
    const logDir = join(process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude'), 'LIFEOS/MEMORY/OBSERVABILITY');

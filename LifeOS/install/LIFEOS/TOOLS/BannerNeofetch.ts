...
function createNeofetchBanner(): string {
  ...
  statsLines.push(`${gradientName} ${UI.muted}@${RESET} ${UI.subtext}LifeOS${RESET}`);
...
function createCompactBanner(): string {
  ...
  lines.push("");
  lines.push(`  ${UI.dim}╭──${RESET} ${paiGradient} ${UI.dim}│${RESET} ${UI.subtext}LifeOS${RESET} ${UI.dim}──────╮${RESET}`);

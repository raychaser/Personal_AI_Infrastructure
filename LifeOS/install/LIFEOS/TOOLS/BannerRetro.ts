...
// Full ASCII art isometric LifeOS cube - detailed version
const LIFEOS_LOGO_FULL = [
  "          __________________",
  "         /\\                 \\",
  "        /  \\   P  P  P       \\",
  "       /    \\  P  P  P        \\",
...
function countLearnings(): number {
  const learningDir = join(CLAUDE_DIR, "LIFEOS/MEMORY/LEARNING");
...
function createRetroBanner(): string {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const brandingText = "  LifeOS | LifeOS  ";
  const brandingWidth = brandingText.length + 2;

  lines.push(`${a}${BOX.dtl}${BOX.dh.repeat(brandingWidth)}${BOX.dtr}${RESET}`);
  lines.push(`${a}${BOX.dv}${RESET} ${g}${BOLD}LifeOS${RESET} ${gd}|${RESET} ${h}LifeOS${RESET}  ${a}${BOX.dv}${RESET}`);
...
function createPureASCIIBanner(): string {
  ...
  lines.push(`${gd}+======================================+${RESET}`);
  lines.push(`${gd}||${RESET}  ${g}LifeOS${RESET} ${gd}|${RESET} ${h}LifeOS${RESET}  ${gd}||${RESET}`);
  lines.push(`${gd}+======================================+${RESET}`);

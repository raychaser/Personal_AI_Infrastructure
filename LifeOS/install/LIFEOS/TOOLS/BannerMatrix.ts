 ...
 *   BOTTOM: Glitched branding + LifeOS in dripping Matrix style
 *
...
// =============================================================================
// LifeOS Logo in Matrix Style (ASCII that emerges from rain)
// =============================================================================

// Large LifeOS letters that will "emerge" from the rain
const LIFEOS_MATRIX_LOGO = [
  " ██████╗   █████╗  ██╗",
  " ██╔══██╗ ██╔══██╗ ██║",
  " ██████╔╝ ███████║ ██║",
  " ██╔═══╝  ██╔══██║ ██║",
...
// Dripping/melting LifeOS effect
const LIFEOS_DRIP = [
  "██████╗  █████╗ ██╗",
  "██╔══██╗██╔══██╗██║",
  "██████╔╝███████║██║",
...
// Compact LifeOS logo for smaller modes
const LIFEOS_COMPACT = [
  "┌───┐┌───┐┌─┐",
  "│ ┌─┘│ ┌─┤│ │",
  "│ │  │ ├─┤│ │",
  "└─┘  └─┘ ┘└─┘",
...
function createNanoBanner(stats: SystemStats): string {
  ...
  return `${d}ｱ${RESET}${g}LifeOS${RESET}${d}ｲ${RESET} ${w}${stats.name}${RESET} ${g}[ON]${RESET}`;
}
 ...
 * LEFT: LifeOS logo with rain | RIGHT: System stats
 * BOTTOM: LifeOS dripping + branding
 */
function createNormalBanner(stats: SystemStats): string {
  ...
  // Dimensions
  const logoWidth = 40;  // Width for LifeOS logo + rain
  ...
  // LifeOS logo lines with rain surrounding
  const paiLines = LIFEOS_MATRIX_LOGO.map((line, i) => {
  ...
  // Frame bottom
  const bottomFrame = `${f}${BOLD}${"=".repeat(dividerCol - 1)}[${RESET}${p}LifeOS${RESET}${f}${BOLD}]${"=".repeat(width - dividerCol - 5)}${RESET}`;

#!/usr/bin/env bun
/**
 * LifeOS Installer v6.0 — Welcome MP3 Generator
...
async function generateWelcome() {
  ...
  if (!apiKey) {
    ...
    if (!process.env.ELEVENLABS_API_KEY) {
      console.error("Error: ELEVENLABS_API_KEY not found in environment or ~/.config/LIFEOS/.env");
  ...
  const text = "Welcome to LifeOS. <break time=\"1.0s\" /> Magnifying human capabilities.";
